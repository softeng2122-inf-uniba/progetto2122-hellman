...
final class GameManager {
     ...
     * @throws GameException Eccezione sollevata nel caso in cui la
     *                       parola inserita non sia adeguata
     *                       o nel caso in cui il gioco sia configurabile.
     */
    static void setSecretWord(final Game game,
    final String secretWord) throws GameException {
        ...
        if (game.isConfigurable()) {
            if (game.getNumberLetter() > secretWord.length()) {
                ...
                + " numero troppo elevato di caratteri.");
            } else if (!Pattern.matches("[a-zA-Z]+", secretWord)) {
                throw new GameException("La parola contiene "
     ...
     */
    static String getSecretWord(final Game game) {
        return game.getSecretWord();
    }

    /**
     * Metodo per l'avvio del gioco instanziando il game attuale con i valori
     * del game precedentemente configurato.
     ...
     *                       correttamente.
     */
    static void startGame(final Game currentGame,
    final Game configuratedGame) throws GameException {

        if (currentGame.getSecretWord().equals("")) {
            if (configuratedGame != null
            && !configuratedGame.getSecretWord().equals("")) {
                currentGame.resetGame();
                currentGame.setSecretWord(configuratedGame.getSecretWord());
                currentGame.disableConfigurable();

            } else {
                throw new GameException("Non è ancora stata inserita"
                + " la parola segreta!");
            }
     ...
     * @throws GameException Eccezione che controlla il corretto
     *                       funzionamento del metodo.
     */
    static void backGame(final Game currentGame) throws GameException {
        if (currentGame.getSecretWord().equals("")
            || currentGame.isConfigurable()) {
    ...
    /**
     * Metodo che conta il numero di occorrenze di ciascuna lettera
     * della parola segreta.
     *
     * @param lettersQuantity numero di occorrenze per lettera della
     *                        parola segreta.
     * @param secretword      valore della parola segreta.
     */
    private static void countLetters(
        final Map<Character, Integer> lettersQuantity,
    ...
    /**
     * Metodo che verifica quali lettere sono nella giusta posizione
     * e ne ritorna la quantità.
     *
     * @param secretword      valore della parola segreta.
     * @param word            valore del tentativo inserito.
     * @param lettersQuantity numero di occorrenze per lettera della
     *                        parola segreta.
     * @param formats         array di formati per i caratteri del tentativo
     *                        inserito.
     * @return il numero di lettere nella posizione
     *         corretta del tentativo inserito.
     */
    private static int matchRightPositionCharacters(final String secretword,
        final String word, final Map<Character, Integer> lettersQuantity,
        final int[] formats) {
    ...
    /**
     * Metodo che verifica quali lettere sono nella posizione
     * sbagliata o assente.
     *
     * @param secretword      valore della parola segreta.
     * @param word            valore del tentativo inserito.
     ...
     *                        inserito.
     */
    private static void matchWrongCharacters(final String secretword,
        final String word, final Map<Character, Integer> lettersQuantity,
        final int[] formats) {
        for (int i = 0; i < word.length(); i++) {
     ...
     * @return Costante che stabilisce lo stato del game e la lista
     *         aggiornata dei tentativi effettuati.
     * @throws GameException Eccezione sollevata nel caso in cui la parola
     *                       inserita non sia adeguata o nel caso in cui
     *                       il gioco sia configurabile.
     */
    static Pair<Integer, List<Word>> makeTry(final Game currentGame,
                                            final String word)
    throws GameException {
        ...
        if (!currentGame.isConfigurable()) {
            ...
            if (currentGame.getNumberLetter() > word.length()) {
                throw new GameException("La parola inserita contiene un"
                + " numero insufficiente di caratteri.");
            } else if (currentGame.getNumberLetter() < word.length()) {
                throw new GameException("La parola inserita contiene un numero"
                + " troppo elevato di caratteri.");
            } else if (!Pattern.matches("[a-zA-Z]+", word)) {
                throw new GameException(
                    "La parola contiene caratteri non ammessi.");
            } else {
                ...
                Map<Character, Integer> lettersQuantity =
                new HashMap<Character, Integer>(currentGame.getNumberLetter());

                countLetters(lettersQuantity, currentGame.getSecretWord());

                if (matchRightPositionCharacters(currentGame.getSecretWord(),
                    word, lettersQuantity, formatsArray)
                    != currentGame.getNumberLetter()) {

                    matchWrongCharacters(currentGame.getSecretWord(), word,
                                         lettersQuantity, formatsArray);
                ...
                if (result == Helper.GAME_LOSE
                    && currentGame.getNumberTrys() != currentGame.getMaxTry()) {
                ...
                if (result == Helper.GAME_LOSE || result == Helper.GAME_WIN) {
                    ...
                    if (result == Helper.GAME_LOSE) {
                        trys.add(new Word(getSecretWord(currentGame), formats));
                    }
                    currentGame.resetGame();
                }
     ...
     */
    static boolean isGameStarted(final Game game) {
